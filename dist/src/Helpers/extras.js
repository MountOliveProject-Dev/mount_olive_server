...
exports.getCurrentDate = exports.GOOGLE_DRIVE_PRIVATE_KEY = void 0;
exports.log = log;
...
// 
async function log(RequestType, message, type, detail) {
    const prisma = server_1.default.app.prisma;
    const timestamp = (0, exports.getCurrentDate)();
    if (detail) {
        const logMessage = await (0, Helpers_1.executePrismaMethod)(prisma, "log", "create", {
            data: {
                message: message,
                type: type,
                detail: detail,
        ...
        console.log(`[${timestamp}] ${type}: ${message} - ${detail}`);
    }
    else {
        const logMessage = await (0, Helpers_1.executePrismaMethod)(prisma, "log", "create", {
            data: {
