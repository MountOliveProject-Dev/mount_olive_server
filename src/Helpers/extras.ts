...
import { LogType, RequestType } from "./types";
...
// 
export async function log(RequestType: RequestType,message: string, type: LogType, detail?: string){
  const prisma = server.app.prisma;
  const timestamp = getCurrentDate();
    if(detail){
      const logMessage = await executePrismaMethod(prisma, "log", "create", {
        data: {
          message: message,
          type: type,
          detail: detail,
